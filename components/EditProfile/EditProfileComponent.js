...
const EditProfileComponent = () => {
    ...
    if (userId) {
      ...
      onValue(userRef, (snapshot) => {
        ...
        if (data) {
          ...
          setLocation(
            locationOptions.find((opt) => opt.value === data.location) || null
          );
          setAbout(data.about || "");
          setSelectedOption(data.displayNameFormat || "");
        }
      });
    }
  }, [userId]);
  ...
  const handleSubmit = async () => {
    ...
    const profileData = {
      ...
      about,
      displayNameFormat: selectedOption, // Save the display name format
    };
